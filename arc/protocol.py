...
class ArcServerProtocol(Protocol):
    ...
    def setWeather(self,weather):
        if self.supports_cpe and 'EnvWeatherType' in self.extensions:
            if weather in WEATHER.keys():
                weather = WEATHER[weather]
            elif weather not in WEATHER.values():
                weather = 0
            self.sendPacked(TYPE_ENVSETWEATHER, weather)

    def EnvSetColor(self,vari,red,green,blue):
        if self.supports_cpe and 'EnvColors' in self.extensions:
            self.sendPacked(TYPE_ENVSETCOLOR,int(vari),int(red),int(green),int(blue))
            print("Sent %s %s %s %s" % (str(vari),str(red),str(green),str(blue)))

    def setLightColor(self,re,gr,bl,shad=False):
        norm = [re,gr,bl]
        if shad:
            sre = int(float(re)*.7)
            sgr = int(float(gr)*.7)
            sbl = int(float(bl)*.7)
            self.EnvSetColor(3,sre,sgr,sbl)
        self.EnvSetColor(4,norm[0],norm[1],norm[2])
        ...
        while self.buffer:
            ...
            if type == TYPE_INITIAL:
                ...
                if self.ext_on >= self.extensionamount:
                    print("CLIENT SUPPORTS: %s" % ', '.join(self.extensions))
                    self.sendPacked(
                        TYPE_INITIAL,
                        7, # Protocol version
                        self.packString(self.factory.server_name),
